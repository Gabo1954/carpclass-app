...
export class AuthService {
  ...
  getUser(): Observable<any> {
    const token = localStorage.getItem(this.authTokenKey);
    if (!token) return throwError(() => new Error('No se encuentra el token'));

    const decoded = this.decodeToken(token);
    if (!decoded || !decoded.sub) return throwError(() => new Error('Token inválido'));

    // Obtener el usuario basado en el id
    return this.http.get<any[]>(this.apiUrl).pipe(
      map(usuarios => {
        const user = usuarios.find(u => u.id === decoded.sub);
